...
export class GuardrailModule {
  ...
  static forRoot(options: GuardrailModuleOptions): DynamicModule {
    ...
    const providers: Provider[] = [
      {
        provide: Guardrail,
        useFactory: (): Guardrail => {
          // Create NestJS logger adapter using default NestJS Logger
          // If logger is not provided in config, use NestJS logger
          const loggerToUse =
            guardrailConfig.logger ?? new NestJSLoggerAdapter(new Logger("Guardrail"));
    ...
    // Add GuardrailLogger and route protection logger if enabled
    if (options.showRouteProtection ?? options.debug) {
      // Add GuardrailLogger provider
      providers.push({
        provide: GuardrailLogger,
        useFactory: (): GuardrailLogger => new GuardrailLogger(),
        inject: [],
      });

      // Add RouteProtectionLogger provider
      providers.push({
        ...
        useFactory: (
          ...
          return new RouteProtectionLogger(
            reflector,
            options,
            discoveryService,
            metadataScanner,
        ...
        inject: [Reflector, DiscoveryService, MetadataScanner, GuardrailLogger],
      });
    }
    ...
    return {
      ...
      exports: [
        Guardrail,
        ...((options.showRouteProtection ?? options.debug)
          ? [GuardrailLogger, RouteProtectionLogger]
          : []),
      ],
    };
  }
